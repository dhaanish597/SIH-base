...
export function StudentHomeworkAssignments() {
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold text-gray-900 mb-4">Homework & Assignments</h2>
      <div className="bg-white rounded-xl p-4 shadow-md border border-gray-100 overflow-x-auto">
        {items.length === 0 ? (
          <p className="text-gray-700">No homework or assignments yet.</p>
        ) : (
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {items.map(item => (
                <tr key={item.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-sm text-gray-700 capitalize">{item.type}</td>
