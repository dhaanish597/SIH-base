...
export function StudentHomework() {
  ...
  useEffect(() => {
    const all = listTasks();
    setItems(all.filter(t => t.type === 'homework'));
  }, []);
  ...
  // Treat overdue items as "recently completed" for now (until real completion tracking exists)
  ...
  function progressUntilDue(dueISO: string) {
    // Synthetic progress: from assigned (now-3d) to due
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        {!isEmpty && (
          <Link to="/quizzes" className="text-indigo-600 hover:text-indigo-700 text-sm font-medium">Practice Quizzes →</Link>
        )}
      </div>
        ...
        <div className="bg-white rounded-xl p-8 shadow-md border border-gray-100">
          <div className="flex flex-col md:flex-row items-center gap-8">
            <div className="w-full md:w-1/2">
              <div className="aspect-[4/3] w-full rounded-lg bg-gradient-to-br from-indigo-50 via-emerald-50 to-rose-50 border border-gray-100 flex items-center justify-center">
                <div className="text-6xl select-none">📚</div>
              </div>
            </div>
            <div className="w-full md:w-1/2">
              <h3 className="text-xl font-semibold text-gray-900 mb-2">No homework… yet!</h3>
              <p className="text-gray-600 mb-4">Stay curious and keep learning. Try a quick quiz or explore lessons while you wait for new homework.</p>
              <div className="flex flex-wrap gap-3">
                <Link to="/quizzes" className="inline-flex items-center px-4 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 shadow">Take a Quiz</Link>
                <Link to="/lessons/science/chapter1" className="inline-flex items-center px-4 py-2 rounded-lg border border-gray-300 text-gray-800 hover:border-indigo-500 hover:text-indigo-700">Browse Lessons</Link>
              </div>
            </div>
          </div>

          <div className="mt-8">
            <h4 className="text-sm font-semibold text-gray-800 mb-3">Upcoming homework</h4>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[0,1,2].map((i) => (
                <div key={i} className="border rounded-lg p-4 bg-gray-50">
                  <div className="text-sm text-gray-500">Placeholder</div>
                  <div className="font-medium text-gray-900">Math practice set</div>
                  <div className="text-xs text-gray-500 mb-2">Due soon</div>
                  <div className="w-full h-2 bg-gray-200 rounded">
                    <div className="h-2 bg-indigo-500 rounded" style={{ width: `${20 + i*15}%` }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-6">
          ...
          {upcoming.length > 0 && (
            <div className="bg-white rounded-xl p-4 shadow-md border border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Upcoming homework</h3>
